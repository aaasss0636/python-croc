...
def SampleCurves(sample, N=500):
    ...
    std_deviation = reduce(lambda a,b:a+b, ((x - average) ** 2 for x in AREA) ) / ( N - 1 )
...
def BEDROC(scoreddata, alpha):
    ...
    out['BEDROC'] = (out['area'] - out['min_area']) / (out['max_area'] - out['min_area'])
    return out
    ...
    def sweep_threshold(self, tie_mode = "smooth"):
        ...
        * sample - randomly shuffle the ties.
        ...
        if self.num_pos == 0 or self.num_neg == 0:
            raise AssertionError("There must be at least one positive and one negative example. This data has %i positive(s) and %i negative(s)."%(self.num_pos, self.num_neg))
        ...
        def ignore_ties(ties): return ties

        def sample_ties(ties): return random.sample(ties, len(ties))
        ...
        else: raise ValueError("tie_mode must equal 'smooth_ties', 'ignore_ties', or 'sample_ties'.")
        ...
        for S, Ls in SCORES:
            ...
            for L in Ls:
                ...
                if int(TP) == TP:
                    ...
                    yield float(TP.evalf()), float(TN.evalf()), float(FP.evalf()), float(FN.evalf())
...
class Curve:
    ...
    def append(self, x, y):
        ...
        if len(self) >= 1 :
            ...
            if (x,y) == self[-1]: return #no need to append duplicate point
    ...
    def __add__(self, other): 
        ...
        c1 = list(self); c2 = list(other); C = Curve()

        last_p1 = c1.pop(0); last_p2 = c2.pop(0) 
        ...
        while c1 or c2:
            ...
            if c1[0][0] == x: p1 = c1.pop(0)
            else: p1 = (x, ((c1[0][0] - x) * last_p1[1] + (x - last_p1[0]) * c1[0][1]) / (c1[0][0] - last_p1[0]) )

            if c2[0][0] == x: p2 = c2.pop(0)
            else: p2 = (x, ((c2[0][0] - x) * last_p2[1] + (x - last_p2[0]) * c2[0][1]) / (c2[0][0] - last_p2[0]) )
